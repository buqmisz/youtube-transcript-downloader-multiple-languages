...
def get_transcript():
    ...
    language_code = request.args.get('language', 'en')  # default to English
    app.logger.info(f"Video URL: {video_url}, Format: {format_type}, Language: {language_code}")
    ...
    try:
        app.logger.info(f"Fetching transcript for video ID: {video_id} in language {language_code}")
